...
def fill_in_the_fields(driver, locator, field_value):
    ...
    print("Выделяем поле ввода данных")
    #Удаляем содержимое поля
    field.send_keys(Keys.DELETE)
    print("Очищаем поле ввода данных")
...
def click_login_button(driver, locator): # Нажатие кнопки Login
    ...
    driver.find_element(*locator[0]).send_keys(Keys.ENTER)
    # Вывод сообщения в консоль
...
def try_to_close_error_button(driver, locator_button, locator_message, test_name):
    ...
    message = get_error_message(driver, locator_button)
    if message is not None:
        driver.find_element(*locator_button[0]).send_keys(Keys.ENTER)
